...
/**
 * Servlet implementation class JudgeExperienceServlet
 */
@WebServlet("/JudgeExperienceServlet")
public class JudgeExperienceServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public JudgeExperienceServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("------------------------judgeExperience.html--------------------------------------");
		PrintWriter writer = response.getWriter();
		UserService userService = new UserService();
		User user = null;
		Map<String, Object> map  = new HashMap<String, Object>();
		JudgeCompetitionService judgeCompetitionService=new JudgeCompetitionService();
		int userId = -1;
		//前端获取传入的data
		String userid = null;
		...
		if (user != null) {//插入成功
			...
			List<JudgeCompetition> judgeCompetitions=judgeCompetitionService.queryJudgeExpByUserId(userId);
			if(judgeCompetitions!=null){
				map.put("degreeRecords", judgeCompetitions);
			}else{
