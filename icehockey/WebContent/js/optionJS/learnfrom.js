	...
	$.post(LEARNFROMURL, data, function(result) {
		...
		if (jsonReturn.result == "0") {
			...
			if (jsonReturn.roleName != '教练') {
				if (jsonReturn.coachPlayer.length > 0) {
					for (var i = 0; i < jsonReturn.coachPlayer.length; i++) {
						hresult[i] = jsonReturn.coachPlayer[i];
						// alert("姓名："+hresult[i]);
						trStr += '<tr class="example">';// 拼接处规范的表格形式
						trStr += '<td align="center">教练：'
								+ hresult[i].coachName + '</td>';// 数据表的主键值
						trStr += '<td align="center">' + hresult[i].bestScore
								+ '</td>';// 对应数组表的字段值
						trStr += '</tr>';
						$("#tbody").html(trStr);
					}
				...
				if (jsonReturn.coachPlayer.length > 0) {
					for (var i = 0; i < jsonReturn.coachPlayer.length; i++) {
						...
						// alert("姓名："+hresult[i]);
						trStr += '<tr class="example">';// 拼接处规范的表格形式
						trStr += '<td align="center">球员：'
								+ hresult[i].playerName + '</td>';// 数据表的主键值
						trStr += '<td align="center">' + hresult[i].bestScore
								+ '</td>';// 对应数组表的字段值
						trStr += '</tr>';
						$("#tbody").html(trStr);
